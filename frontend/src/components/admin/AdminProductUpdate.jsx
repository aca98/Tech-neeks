...
import { getProduct, updateProduct } from "../../services/httpService";
import errorMessages from "../../utills/errorMessages";
import AdminAttributeInput from "./AdminAttributeInput";
...
class AdminProductUpdate extends Component {
  ...
  async componentDidMount() {
    ...
    product.product.attribute.forEach((value, index, array) => {
      array[index].unice = value.id;
    });
  ...
  checkIfDuplicate = (state) => {
    let faultyFields = state.product.attribute.filter(
      (value, index, array) =>
        array.filter((val2) => val2.name === value.name).length > 1
    );
  ...
  saveProduct = async () => {
    const serverResponse = await updateProduct(
      this.state.product,
      this.state.images
    );
      ...
      <div className="row mx-2">
        <div className="w-100 row mx-auto">
          ...
          <div className="row m-0 mt-2 w-100">
            ...
            {this.state.product.attribute.length &&
